...
export async function getVisitorStats() {
  const empty = { totalViews: 0, uniqueVisitors: 0, todayViews: 0, weekViews: 0, topPages: [], daily: [] };
  ...
  try {
    ...
    const [totalViews, uniqueRows, todayViews, weekEvents, pageGroups] = await Promise.all([
      prisma.visitorEvent.count(),
      prisma.visitorEvent.findMany({ distinct: ["visitorId"], select: { visitorId: true } }),
      prisma.visitorEvent.count({ where: { createdAt: { gte: today } } }),
      prisma.visitorEvent.findMany({ where: { createdAt: { gte: weekAgo } }, select: { createdAt: true } }),
      prisma.visitorEvent.groupBy({ by: ["path"], _count: { path: true }, orderBy: { _count: { path: "desc" } }, take: 8 })
    ]);
    ...
    const daily = Array.from({ length: 7 }, (_, index) => {
      ...
      return {
        ...
        views: weekEvents.filter((event) => event.createdAt.toISOString().slice(0, 10) === key).length
      };
    });
    ...
    return {
      ...
      todayViews,
      weekViews: weekEvents.length,
      topPages: pageGroups.map((page) => ({ path: page.path, views: page._count.path })),
      daily
    };
  } catch (error) {
